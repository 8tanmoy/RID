...
for ii in `seq -w $miniter $maxiter`; do
    echo $ii
    dfold=data_poatt_$ii
    mkdir $dfold
    cd $dfold
#
    echo $PWD
    sort -k1 -n ../../iter.$ii/02.train/fe.out > fe.out.sort1
#
    cd ../
done
...
for ii in `seq -w $miniter $maxiter`; do
    ...
    cd $dfold
#
    awk '{print 10*$2, 10*$3, $4/(-4.184), 10*$1}' fe.out.sort1 > temp00                        #flipping 2 and 1 because 1 is P-Oattack, 2 is P-Olg
    femin=`sort -nk3 temp00 | head -1 | cut -d ' ' -f3`
    awk -v femin="$femin" '{print $1, $2, $3 - femin, $4}' temp00 > fe.out.sort1.min
    rm temp00
    for jj in `seq 0 $((reso - 1))`; do
        beg=$(( jj * reso * reso  + 1 ))
        end=$(( (jj + 1) * reso * reso ))
        sed -n "${beg},${end}p" fe.out.sort1.min > fe.out.sort1.min.$((jj + 1))
    done
#
    cd ../
done
...
#
    for jj in `seq 1 $reso`; do
        fname=fe.out.sort1.min.$jj
        imname=img_iter_${ii}_${jj}.png
        sed "s|_FILENAME_|$fname|g" ../plot_master_poatt.wl > temp1
...
#
    ...
    convert -delay 20 -quality 100 ${arr[@]} -loop 1 fe_movie_iter_${ii}_poatt.gif
#
    cd ../
done
