...
def value_array (sess, ngrid) :
    ...
    my_grid    = np.zeros((ngrid*ngrid, cv_dim))

    for i in range(ngrid):
        for j in range(ngrid):
                my_grid[i*ngrid + j, 0] = xmin + i * deltax
                my_grid[i*ngrid + j, 1] = ymin + j * deltay
...
def print_array (fname, xx, yy, dd) :
    with open(fname, 'w') as fp :
        lx = len(xx)
        ly = len(yy)
        for ii in range (len(xx)) :
            for jj in range (len(yy)) :
                fp.write ("%f %f %f\n" % (xx[ii], yy[jj], dd[ii*lx + jj]) )
            #fp.write ("\n")
...
def _main () :
    ...
    for ii in args.model :
        ...
        with tf.Session(graph = graph) as sess:
            xx, yy, dd = value_array (sess, args.numb_grid)
            dd *= f_cvt
            if count == 0:
                avg0 = dd
            else :
