...
train_name="02.train"
train_files=["model.py", "main.py", "freeze.py", "plot_3d.py"]

mol_name="mol"
mol_files=["conf.gro", "grompp.mdp", "topol.top", "dist.ndx", "dist.sh", "index.ndx", "dftb_in.hsd", "geom.gen", "plm.md.dat"] #tanmoy D1003 delete alanine.itp

def get_bash():
    bash_line = '#!/bin/bash -l \n'
    bash_line += '#$ -l h_rt=2:00:00 \n'
    bash_line += '#$ -l avx \n'
    bash_line += '#$ -j y \n'
    bash_line += f'#$ -N {res_jobname} \n'
    bash_line += '#$ -pe omp 1 \n\n'
    bash_line += 'module purge \n'
    bash_line += 'module load python3/3.7.9 \n' 
    bash_line += 'module load cmake/3.19.4 \n'
    bash_line += 'module load openmpi/3.1.4_gnu-8.1 \n'
    bash_line += 'module load scalapack/2.0.2_gcc-8.1_openmpi-3.1.4 \n'
    bash_line += 'module load openblas/0.3.7 \n'
    bash_line += 'module load gcc/8.1.0 \n'
    bash_line += 'source /projectnb/cui-buchem/tanmoy/packages/gromacs-dftbplus-ml-rl/install/bin/GMXRC \n'
    bash_line += 'export PLUMED_USE_LEPTON=yes \n\n'
    #bash_line += 'sleep 60\n'
    return bash_line

def check_current_jobs(sec=60):
    print("sleeping for 15 mins. adjust based on experience.")
    time.sleep(60*15)
    while True:
        jobs = sp.Popen("qstat -u tanmoy | grep '" + res_jobname + "' | awk '{print $1}'", shell=True, stdout=sp.PIPE).communicate()[0].decode('utf-8').split('\n')
        jobs.remove('')
        running = sp.Popen("qstat -u tanmoy -s r | grep '" + res_jobname + "' | awk '{print $1}'", shell=True, stdout=sp.PIPE).communicate()[0].decode('utf-8').split('\n')
        running.remove('')
        if len(jobs) == 0:
            #time.sleep(60*5)
            break
        else:
            print("running jobs: ", len(running), " / ", len(jobs))
            time.sleep(sec)
...
def run_res (iter_index,
    ...
    for work_path in all_task:
        ...
        bash_line = get_bash()
        bash_line += gmx_prep_cmd + '\n'
        bash_line += gmx_run_cmd  + '\n'
        with open('sub_res.sh','w') as subfile:
            subfile.write(bash_line)
        #time.sleep(0.5)
        sp.run("qsub sub_res.sh", shell=True)
        os.chdir(base_path)
    check_current_jobs()
...
def run_train (iter_index, 
    ...
    train_path = iter_name + "/" + train_name + "/"  
    base_path = os.getcwd() + "/"
    plot_cmd  = "python3 plot_3d.py -m *.pb -n 100 -o fe_%03d.out" % iter_index #tanmoy
    ...
    #log_task(plot_cmd)
    #sp.check_call(plot_cmd, shell=True)
    os.chdir (base_path)
