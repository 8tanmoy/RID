...
def _main () :
    ...
    parser.add_argument("--output-dist", default="sel.dist.out", type=str, 
                        help="output selecte dist") #tanmoy D1003 angle->dist
    ...
    np.savetxt (args.output_dist, np.reshape(sel, [-1,cv_dim]))
    print ("max std: %f, min std: %f, avg std %f" % ( np.max(avg_std), np.min(avg_std), np.average (avg_std)))
    print ("number of dists greater than %f is %d" % (args.threshold, len(idx)))
